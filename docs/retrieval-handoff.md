...
1. **판례 원문 교체** — 현재 26건은 요약문입니다. 교체하면 모든 수치를 다시 잽니다.

2. **가이드 문서 확대와 라우팅 재평가** — 현재는 공식 문서 2건·6청크뿐입니다.
   서로 다른 사용자 의도를 다루는 공식 안내를 추가하고, 관련·무관 질문이 충분히
   모이면 주제 조건과 유사도 문턱을 다시 측정합니다.

3. **실데이터 재측정** — 법령이나 안내가 늘면 전부 다시 잽니다. 상가건물임대차보호법 하나를
   추가했을 때 MRR 이 0.581 → 0.446 으로 떨어진 적이 있습니다.

### 평가 자체를 손봐야 하는 일

4. **holdout-v2 평가셋** — 현재 holdout 18문항의 1회 측정은 끝났습니다. 다음 검색
   설정 변경을 검증할 때는 기존 결과에 맞추지 말고 새 문항을 봉인해 측정합니다. 현재
   결과는 Hit@1 66.7% / Hit@5 94.4%
...
```
전체 테스트  189 passed, 2 skipped
진입점 테스트 49 passed · 안내 수집·적재 18 passed
```
