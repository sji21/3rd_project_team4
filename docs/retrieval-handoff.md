...
result.guides                # list[Evidence] — 공식 안내 2건 (HUG·국세청)
...
안내 코퍼스가 6청크(문서 2건)뿐이라 **어떤 질문에도 그중 2건이 반환됩니다.**
...
| **기본값** 법령 5 + 판례 5 + 안내 2 | 약 6,300자 | 3,100 |

안내 2건이 약 1,400자를 더합니다. 안내가 필요 없는 화면이면 `k_guide=0` 으로 끄면
그만큼 줄어듭니다.
...
4. **holdout 평가셋** — 끝났습니다. 처음 보는 18문항에서 Hit@1 66.7% / Hit@5 94.4%
   로, dev 수치가 크게 부풀려졌다는 신호는 나오지 않았습니다. `docs/eval-holdout.md`.
...
```
전체 테스트  179 passed, 2 skipped
진입점 테스트 42 passed · 안내 적재 15 passed
```
