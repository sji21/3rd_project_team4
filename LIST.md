...
| [ ] | PATCH-022 | 리뷰 보완 중 | chzhdndb90 | `feat/patch-022-case-data-ingestion` | 기능 추가 | 높음 | 공식 판례 상세 원천을 표준 판례 레코드로 안전하게 변환 | 공개 API 검증 원천만 표준 레코드로 발행하고, 오류·제외·수동 검토·동일성 충돌을 분리하며, 사건번호 중복과 SQLite·청크 적재 규격 테스트를 통과해야 한다. 적재 건수는 검증 원천과 범위 판정 결과로 매번 산출한다. | PATCH-018, PATCH-019 | PR #10 | 2026-08-30 | - | `7e178f7` |
...
- 현재 리뷰 보완 중 패치: `PATCH-022` (공식 판례 원천 JSONL 안전 변환·재생성 경로, PR #10)
