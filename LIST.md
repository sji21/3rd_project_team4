...
| [ ] | PATCH-019 | 진행 중 | Codex | `feat/patch-019` | 기능 추가 | 중간 | PATCH-018 표준 판례 청크 기반 임베딩 비교·평가 통합 | 공통 `cases.jsonl`·메타데이터·Chroma 색인 구조를 사용해 판례 전용 KURE 평가와 로컬·Hugging Face 임베딩 비교를 재현 가능하게 실행하고, 운영용 통합 인덱스를 변경하지 않아야 한다 | PATCH-018 | - | 2026-08-29 | - | - |
| [ ] | PATCH-020 | 진행 중 | sji21 | `feat/patch-020-holdout-eval` | 검증 | 높음 | 홀드아웃 평가셋 봉인과 1회 측정 | 저장소를 본 적 없는 출처가 쓴 질문으로 평가셋을 만들고, 정답은 법령 원문만 보고 붙이며, 범위 밖 판정을 점수 확인 전에 끝내고, 결과를 보고 설정을 고치지 않아야 한다 | PATCH-018 | - | 2026-08-29 | - | - |
...
- 현재 진행 패치: `PATCH-019` (판례 전용 비교·평가 통합)
- 현재 진행 패치: `PATCH-020` (검색 담당, 홀드아웃 평가)
- `PATCH-015`는 생성 담당이 완료해 병합됨(PR #2)
- 임베딩 모델: `nlpai-lab/KURE-v1` (1024차원) — 모델 비교 결과로 확정
- 검증 결과: 전체 테스트 157개 통과(스킵 2, 사전 존재하던 Windows 환경변수 길이 오류 2건은 `tests/test_pdf_extraction.py` 소관)·`pip check`·135개 검색 청크 규격 검사 통과
...
| 과제 | 막고 있는 것 |
| --- | --- |
| 가이드 문서 적재 (`guides` + export + 색인) | HUG·국세청 **원문** 수집. 현재 실험 코퍼스의 2건은 평가 질문 작성 다음 날 쓴 요약문이라 그대로 편입하면 수치만 오른다 |
