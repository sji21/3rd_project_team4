...
- 검토 완료 패치: `PATCH-033` (민법 임대차 조문 추가 검토). 결론은 1순위 5조문(`min5`)이며 dev 법령 Recall@3 29/31·판례 순위 변동 0건으로 회귀가 없다. 2순위 5조문은 교란만 늘어 넣지 않는다. 제621조는 주임법 제3조의2·제3조의4 가 본문에서 인용하는데 코퍼스에 없어 생성 단계에서 환각으로 잡힌 적이 있으나, 검색 지표로는 손해라 보류한다. 코퍼스 실제 반영은 재인덱싱이 따르므로 별도 패치로 연다. 자세한 내용은 `docs/minbeop-corpus-review-20260901.md`
...
| 민법 `min5` 코퍼스 실제 반영 (`fetch_law_mock.LAWS` 조문 필터 + 재인덱싱) | 없음 — PATCH-033 에서 회귀 없음을 확인했다 |
