...
| [ ] | PATCH-017 | 진행 중 | sji21 | `fix/patch-017-index-scope` | 버그 수정 | 높음 | Chroma 색인의 삭제 범위를 입력의 doc_type 으로 한정 | 법령과 판례를 같은 컬렉션에 두고 각각 따로 재색인해도 서로의 문서가 지워지지 않아야 하고, 같은 doc_type 안에서는 빠진 청크가 계속 정리되어야 하며, 빈 입력이 컬렉션을 비우지 않아야 한다 | PATCH-014 | - | 2026-08-28 | - | - |
...
- 검증 결과: 전체 테스트 148개 통과(스킵 2, 사전 존재하던 Windows 환경변수 길이 오류 2건은 `tests/test_pdf_extraction.py` 소관)·`pip check`·135개 검색 청크 규격 검사 통과
