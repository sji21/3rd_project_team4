...
| [ ] | PATCH-016 | 진행 중 | sji21 | `feat/patch-016-hybrid-rrf` | 기능 추가 | 높음 | BM25와 임베딩 검색을 RRF로 결합 | 점수 척도가 다른 두 검색기를 순위로 합치고, 같은 `search(query, k, where)` 인터페이스를 유지하며, 각 검색기가 서로 다른 문항에서 실패하던 것이 결합 후 해소되어야 한다 | PATCH-014 | - | 2026-08-28 | - | - |

## 현재 작업 경계

- 현재 완료 패치: `PATCH-014` (PR #1 병합, `PATCH-013` 포함)
- 현재 진행 패치: `PATCH-016` (검색 담당)
- `PATCH-015`는 생성 담당이 완료해 병합됨(PR #2)
- 임베딩 모델: `nlpai-lab/KURE-v1` (1024차원) — 모델 비교 결과로 확정
- 검증 결과: 전체 테스트 108개 통과(스킵 2, 사전 존재하던 Windows 환경변수 길이 오류 2건은 `tests/test_pdf_extraction.py` 소관)·`pip check`·135개 검색 청크 규격 검사 통과
