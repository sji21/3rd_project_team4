...
| [ ] | PATCH-014 | 진행 중 | sji21 | `feat/patch-014-kure-chroma-index` | 기능 추가 | 높음 | 임베딩 모델 확정과 Chroma 적재·검색 연결 | 같은 평가셋으로 임베딩 모델을 비교해 근거와 함께 하나를 고르고, 청크를 해당 모델로 임베딩해 Chroma에 멱등 적재하며, 같은 `search(query, k, where)` 인터페이스로 조회해 메모리 계산과 같은 결과가 나와야 한다 | PATCH-013 | - | 2026-08-28 | - | - |

## 현재 작업 경계

- 현재 완료 패치: `PATCH-012`
- 현재 진행 패치: `PATCH-013`, `PATCH-014` (검색 담당)
