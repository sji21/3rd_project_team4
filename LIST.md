...
| [x] | PATCH-022 | 완료 | chzhdndb90 | `feat/patch-022-case-data-ingestion` | 기능 추가 | 높음 | 공식 판례 상세 원천을 표준 판례 레코드로 변환 | 국가법령정보센터 상세 원천 JSONL에서 207건 표준 `case_records.jsonl`을 재현하고, 사건번호 중복·수동 제외 범위를 일관되게 처리하며, SQLite·청크 적재 규격 테스트가 통과해야 한다 | PATCH-018, PATCH-019 | - | 2026-08-30 | 2026-08-30 | `7e178f7` |
...
- 현재 완료 패치: `PATCH-022` (공식 판례 원천 JSONL 표준 변환·재생성 경로)
- 현재 진행 패치: `PATCH-020` (검색 담당, 홀드아웃 평가), `PATCH-021` (생성 담당)
