...
| [ ] | PATCH-033 | 검토 완료 | sji21 | `feat/patch-033-minbeop-corpus-review` | 검증 | 중간 | 민법 임대차 조문을 법령 코퍼스에 추가할 수 있는지 검토 | 주택임대차보호법만으로 근거를 찾기 어려운 생활형 질문(시설 고장·수리비·일부 사용 불가·차임 연체)을 위해 민법 임대차 조문 추가를 검토한다. 저장소의 DB·인덱스·청크를 바꾸지 않고 임시 사본에서만 재는 하네스를 제공하며, `base`·`min5`·`min6`·`max11` 네 변형을 같은 임베딩 모델로 비교해 dev 27문항 법령 Recall@3·판례 홀드아웃 순위·신규 20문항 TOP5 변화를 재현한다. Recall 분모는 법령 정답만 세고 법령 검색으로 도달 불가한 안내 정답은 제외한다. 정답 손실 여부와 함께, `DEFAULT_K_LAW=3` 기준으로 **생성 근거에서 빠진 조문**을 변형별로 집계한다. 코퍼스 실제 반영·조건부 라우팅·재인덱싱은 이 패치에 포함하지 않는다. | PATCH-026, PATCH-028 | - | 2026-09-01 | 2026-09-01 | `5663046` |
...
- 검토 완료 패치: `PATCH-033` (민법 임대차 조문 추가 사전 영향 분석). 1순위 5조문(`min5`)을 **조건부 채택 후보**로 본다. dev 27문항의 법령 Recall@3 는 네 변형 모두 29/29(100%)로 정답 손실이 없고 판례 순위 변동도 0건이지만, **평가셋 밖 질문에서 생성 근거가 바뀌는 회귀가 확인됐다** — 신규 10번("계약을 다시 하고 싶으면…")에서 민법 제640조가 3위로 들어와 주임법 제6조가 `DEFAULT_K_LAW=3` 밖으로 밀렸다. 그래서 민법을 법령 코퍼스에 항상 노출하지 말고 `route_law_corpus()` 방식의 조건부 라우팅을 전제로 반영해야 한다. 검색 개선이 실제로 검증된 조문은 제640조 하나뿐이며 제623·626·627·634조는 해당 질문이 평가셋에 없어 미검증이다. 2순위 5조문과 제621조는 넣지 않는다. 실제 적재·라우팅·재인덱싱은 별도 패치로 연다. 자세한 내용은 `docs/minbeop-corpus-review-20260901.md`
...
| 민법 `min5` 실제 반영 (`fetch_law_mock.LAWS` 조문 필터 + 조건부 라우팅 + 재인덱싱) | 생활형 평가 문항. PATCH-033 에서 무조건 노출은 생성 근거를 해치는 것으로 확인돼, 라우팅과 문항을 함께 넣어야 한다 |
| 생활형 질문(시설 고장·수리비·누수·통지)으로 민법 제623·626·627·634조 검색 검증 | 평가 문항. PATCH-033 의 신규 20문항은 전부 주택임대차 질문이라 제640조만 검증됐다 |
