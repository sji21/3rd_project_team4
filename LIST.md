...
| [x] | PATCH-033 | 수정 완료 | sji21 | `feat/patch-033-minbeop-corpus-review` | 검증·기능 추가 | 중간 | 민법 임대차 조문 추가 검토와 조건부 라우팅 구현 | 주택임대차보호법만으로 근거를 찾기 어려운 생활형 질문(시설 고장·수리비·일부 사용 불가·차임 연체)을 위해 민법 임대차 조문 추가를 검토한다. 저장소의 DB·인덱스·청크를 바꾸지 않고 임시 사본에서만 재는 하네스를 제공하며, `base`·`min5`·`min6`·`max11` 네 변형을 같은 임베딩 모델로 비교해 dev 27문항 법령 Recall@3·판례 홀드아웃 순위·신규 20문항 TOP5 변화를 재현한다. Recall 분모는 법령 정답만 세고 법령 검색으로 도달 불가한 안내 정답은 제외한다. 정답 손실 여부와 함께, `DEFAULT_K_LAW=3` 기준으로 **생성 근거에서 빠진 조문**을 변형별로 집계한다. 검토 결과 무조건 노출이 생성 근거를 해치는 것으로 확인돼 **조건부 라우팅 구현까지 이 패치에 포함**한다. 민법 6조문(제623·626·627·629·634·640조)을 기존 법령 BM25 색인에서 분리하고, 결정론적 질문 신호가 맞을 때만 0~2건을 주입하며 법령 3건 예산을 유지한다. 신호어 표를 보지 않고 쓴 문항으로 발동·비발동을 함께 잠그고, 민법 청크가 없으면 올바른 재적재·재인덱싱 명령을 경고한다. 설비명은 실제 고장 증상과 함께 있을 때만 수선의무가 발동하도록 오탐을 차단한다. 유사도 임계값 튜닝은 마감 후 과제로 남긴다. | PATCH-026, PATCH-028 | - | 2026-09-01 | 2026-09-01 | `5663046`, `cdba477`, `323f16f` |
...
- 완료 패치: `PATCH-033` (민법 임대차 조문 조건부 라우팅). 민법 6조문(제623·626·627·629·634·640조)을 **기존 법령 BM25 색인에서 분리**하고, 결정론적 질문 신호가 맞을 때만 0~2건을 주입한다. 생성에 넘기는 법령은 3건으로 유지한다. 색인을 분리한 이유는 검색 시점 필터로는 부족하기 때문이다 — 같은 색인에 있으면 결과에 안 보여도 IDF가 바뀌어 기존 순위가 흔들린다. 회귀: dev 27문항 법령 Recall@3 29/29 유지·TOP3 구성 변경 0건, 법령 holdout TOP3 구성 변경 0건, 판례 순위 변경 0건, 민법 오발동 0건. **유사도 임계값은 튜닝하지 않았다** — 조건이 맞으면 유사도와 무관하게 주입되므로 키워드 규칙의 정확도가 유일한 안전장치다. 그래서 신호어 표를 보지 않고 먼저 쓴 문항으로 발동·비발동을 함께 잠갔고(`CivilNaturalWordingTests`), 민법 청크가 없으면 경고를 남긴다. 민법 후보 내부의 의미 기반 순위화는 평가 문항이 쌓인 뒤 후속 과제로 한다. 자세한 내용은 `docs/minbeop-corpus-review-20260901.md`, `docs/minbeop-conditional-routing-result-20260901.md`
...
| 민법 후보 내부의 의미 기반 순위화 (지금은 조건 충족 시 유사도와 무관하게 주입) | 평가 문항. 후보가 6건뿐이라 임계값을 지금 정하면 그 표본에 맞추는 것이 된다 |
| 봉인된 생활형 홀드아웃으로 민법 라우팅 일반화 검증 | 새 문항. PATCH-033 의 30문항은 라우팅 확정 전에 원문을 봐서 엄밀한 blind holdout 이 아니다 |
| 제640조 요건("연체액이 2기의 차임액에 달하는 때")이 생성 답변에 보존되는지 | 생성 E2E. 검색이 조문을 찾아도 "두 번 연체" 로 뭉개면 틀린 답이 된다 |
| 생활형 질문의 범위 판정 통과 여부 | `abstention._IN_SCOPE_CUES` 와 `SCOPE_JUDGE_SYSTEM` 에 수선·고장·누수·전대 어휘가 없어 검색 이전에 걸린다. 생성 파트와 함께 봐야 한다 |
