...
| [ ] | PATCH-015 | 진행 중 | sji21 | `feat/patch-015-hybrid-rrf` | 기능 추가 | 높음 | BM25와 임베딩 검색을 RRF로 결합 | 점수 척도가 다른 두 검색기를 순위로 합치고, 같은 `search(query, k, where)` 인터페이스를 유지하며, 각 검색기가 서로 다른 문항에서 실패하던 것이 결합 후 해소되어야 한다 | PATCH-014 | - | 2026-08-28 | - | - |

## 현재 작업 경계

- 현재 완료 패치: `PATCH-012`
- 현재 완료 패치: `PATCH-014` (PR #1 병합)
- 현재 진행 패치: `PATCH-015` (검색 담당)
