...
| [x] | PATCH-033 | 검증 완료·적용 보류 | sji21 | `feat/patch-033-minbeop-corpus-review` | 검증·프로토타입 | 중간 | 민법 임대차 조문 추가 검토와 조건부 라우팅 프로토타입 | 주택임대차보호법만으로 근거를 찾기 어려운 생활형 질문(시설 고장·수리비·일부 사용 불가·차임 연체)을 위해 민법 임대차 조문 추가 영향을 검토하고 조건부 라우팅 프로토타입을 구현한다. 민법 6조문(제623·626·627·629·634·640조)을 기존 법령 BM25 색인에서 분리하고, 결정론적 질문 신호가 맞을 때만 0~2건을 주입하며 법령 3건 예산을 유지한다. dev·기존 holdout·판례 회귀와 자연어 발동·비발동 테스트는 통과했지만, 규칙이 조문까지 직접 정해 유사도 백스톱이 없고 새 blind holdout 및 생성 E2E 검증이 남아 있으므로 **이번 제출·기본 시연 구성에는 반영하지 않는다.** 코드는 이 브랜치에 보존한다. 시연에서 생활형 질문 대응이 꼭 필요해질 때만 README 1-1에 따라 민법 6조를 적재·재인덱싱하고 `법령 청크 139건 / Chroma 171건 / 민법 6건`과 대표 질문 E2E를 확인한 뒤 제한적으로 활성화한다. | PATCH-026, PATCH-028 | - | 2026-09-01 | 2026-09-01 | `5663046`, `cdba477`, `323f16f`, `ba9d623` |
...
- 검증 완료·적용 보류 패치: `PATCH-033` (민법 임대차 조문 조건부 라우팅 프로토타입). 기존 법령 BM25와 분리해 회귀를 막고 자연어 발동·비발동 테스트까지 통과했지만, 유사도 백스톱·새 blind holdout·생성 E2E가 없어 **이번 제출과 기본 시연에는 넣지 않는다.** 구현과 평가 근거는 브랜치에 보존한다. 생활형 질문 시연이 꼭 필요한 경우에만 민법 6조 적재·재인덱싱 후 `법령 139 / Chroma 171 / 민법 6` 건수와 대표 질문 E2E를 확인하여 제한적으로 활성화한다. 자세한 내용은 `docs/minbeop-corpus-review-20260901.md`, `docs/minbeop-conditional-routing-result-20260901.md`
...
| 민법 조건부 라우팅의 운영 반영 | 위 두 검증과 생성 범위 판정·답변 E2E가 끝난 뒤 진행한다. 이번 제출에는 미적용하고, 시연상 꼭 필요할 때만 6조 적재·재인덱싱·건수 확인 후 제한적으로 켠다 |
