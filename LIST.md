...
| [ ] | PATCH-002 | 배정됨 | gitcatho | `feat/patch-002-risk-signals` | 기능 추가 | 높음 | 개인정보 마스킹과 등기 위험 신호 규칙 | 주민번호·전화·계좌 마스킹, 갑구·을구 핵심 신호 탐지, 근거 페이지·문구·추가 확인사항 반환, 안전 판정 금지, 규칙 테스트 통과 | PATCH-001 | - | 2026-08-26 | - | pending |
| [ ] | PATCH-003 | 배정됨 | gitcatho | `feat/patch-003-streamlit-registry-check` | 기능 추가 | 높음 | Streamlit PDF 업로드 및 점검 결과 화면 | 동의 기반 PDF 업로드, 상태·신호·근거·체크리스트 표시, 마스킹된 미리보기, 개인정보 제외 JSON 다운로드, Streamlit 앱 테스트 통과 | PATCH-002 | - | 2026-08-26 | - | pending |
...
- 현재 활성 패치: 없음 (`PATCH-002` 시작 대기)
- 다음 허용 범위: 개인정보 마스킹·등기 위험 신호 규칙과 해당 테스트
- 다음 패치 시작 조건: `PATCH-002` 전용 브랜치 전환 및 상태를 `진행 중`으로 갱신
