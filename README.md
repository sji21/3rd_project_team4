...
| 팀원 | 회고 내용 |
| --- | --- |
| 김혜진 | 개발 업무보다 개발 전 기획, 개발 후 테스트가 중요함을 다시 느꼈습니다. 그리고 한 작업을 여러명이 하면 충돌의 위험이 커지는 것을 생각해 각 파트별로 맡아 진행했음에도 충돌이나 예기치 못한 이슈사항을 여러 번 겪었지만, 한 이슈를 빨리 해결하기 위한 팀원들의 화합이 있어 빠른 시일 내 해결이 가능했습니다. |
| 김정재 | 프로젝트를 진행하면서 맡은 부분만 보는 것이 아니라 전체 흐름을 이해하고 서로의 작업을 연결하는 과정이 중요하다는 걸 배웠습니다. 또한 협업 과정에서 문제 원인을 함께 찾아가며, 검색·생성·검증 단계를 나눠 개선하는 방식이 서비스 품질을 높이는 데 중요하다는 점을 느꼈습니다.  |
| 송지섭 | 실제로 프로젝트를 진행하니까 구현이 빠르게 끝날지라도 그것을 검증하고 기능을 확장해나가는 것이 큰 작업이라는 것을 느꼈습니다. 프로젝트 진행간 타협한 문제들을 개선하여 퀄리티를 높이는데 집중해야겠다고 생각했습니다. |
| 신진호 | 모두가 적극적이라 주제 선정부터 실제 프로젝트까지 막힘없이 진행되었던 프로젝트였습니다. 좀더 발전시키면 실용 서비스로 응요할수 있을 만큼 확장성이 있는 프로젝트라 이후 4차 때까지 많은 발전이 기대됩니다. |
| 윤지환 | 청크 생성, Chroma 적재 과정에서 법과 판례에 대한 지식이 부족하여 데이터 구축에 아쉬움이 남았습니다. 데이터 구축을 하기 전, 도메인을 공부하고 해야 한다는 것을 배웠습니다.  또한, Retriever 과정을 직접 진행해보며 임베딩, 평가에 대해 활용할 수 있었습니다. 부족함이 많았던 만큼 배울 수 있는 것도 많았던 프로젝트였고 팀원들과 좋은 분위기 속에 협업 할 수 있던 프로젝트였습니다. |
