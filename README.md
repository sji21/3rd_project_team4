...
현재 Streamlit 화면은 주택임대차 질문을 받으면 공식 법령·판례·기관 안내를 검색하고 로컬 Qwen3 모델이 근거 기반 답변과 출처를 제공하는 챗봇이다. 등기사항증명서·임대차계약서 OCR 점검 모듈도 코드로 제공하지만 현재 챗봇 화면에는 연결하지 않았다. 이 서비스는 개별 계약의 안전 여부를 판정하지 않는다.
