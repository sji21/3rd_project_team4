...
아래 역할은 프로젝트 진행 내용을 기준으로 정리한 초안이며, 최종 발표 전 팀 확인을 거쳐
확정합니다.
...
짧은 질문만으로 첨부 문서를 가리키는 대상이 분명하지 않으면 일반 상담 질문으로 처리될
수 있습니다. 이때는 “업로드한 계약서의 보증금은 얼마야?”, “이 등기 문서의 을구를
설명해줘”처럼 `계약서`, `등기 문서`, `업로드 문서`를 질문에 직접 적으면 문서 근거를
선택할 가능성이 높아집니다.
...
사용자 질문 또는 업로드 문서
            │
            ├─ 문서 추출: PDF 글자 추출 → 필요 시 OCR → 개인정보 마스킹
            │
...
- 짧은 후속 질문에서 업로드 문서가 명확하지 않으면 `계약서`, `등기 문서` 등을 직접
  밝혀야 문서 질문으로 인식될 가능성이 높아짐
