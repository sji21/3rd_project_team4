...
**원문을 그대로 수집합니다**(HUG 2,807자 · 국세청 1,027자 → 7청크). 요약해서 넣으면
...
| 정답 조문이 반환된 근거 안에 있는가 | 안내 없음 | 안내 포함 |
| --- | --- | --- |
| `dev` (25문항) | 96.0% | **100.0%** |
...
> ⚠️ **안내는 관련이 없어도 2건이 따라옵니다.** 코퍼스가 7청크뿐이라 어떤 질문에도
> 그중 2건이 반환됩니다. RRF 점수는 순위만 보므로 걸러낼 수 없고(관련 O·X 모두
> 0.0323~0.0328), 임베딩 코사인은 관련 O의 최저 0.5895와 관련 X의 최고 0.5445가
> 0.045밖에 차이 나지 않습니다. 문서 2건으로 문턱을 정하면 그 표본에 맞추는 것이라
> 넣지 않았습니다. **관련 여부는 프롬프트에서 LLM이 판단하도록 지시해야 합니다** —
> `docs/retrieval-handoff.md` 참고.
